...
__global__ void kernel_channel_max(int num, int channels,
                    int spatial_dim, float *data, float *out, int iteration)
{
...
// __global__ void kernel_channel_subtract(int count,
//     int num, int channels,
//     int spatial_dim, float* channel_max, float* data) {
...
int main(int argc, char *argv[])
{
    ...
    cudaMalloc((void **)&in, count * out_num * in_c * sizeof(float));
    cudaMalloc((void **)&out, count * out_num * in_c * sizeof(float));
    ...
    soft_block.x = 256;
    soft_grid.x = count * out_num / 256;

    printf("[ORI] Running with relu...\n");
    printf("[ORI] soft_grid -- %d * %d soft_block -- %d * %d \n",
            soft_grid.x, soft_grid.y, soft_block.x, soft_block.y);

    cudaErrCheck(cudaEventRecord(startKERNEL));
    checkKernelErrors((kernel_channel_max<<<soft_grid, soft_block>>>(
        out_num, in_c, count, in, out, soft_iter)));
    cudaErrCheck(cudaEventRecord(stopKERNEL));
    cudaErrCheck(cudaEventSynchronize(stopKERNEL));
    cudaErrCheck(cudaEventElapsedTime(&kernel_time, startKERNEL, stopKERNEL));
    printf("[PTB] soft took %f ms\n\n", kernel_time);
    ...
    checkKernelErrors((kernel_channel_max<<<soft_grid, soft_block, 0, streams[1]>>>(
        out_num, in_c, count, in, out, soft_iter)));
    cudaErrCheck(cudaEventRecord(stopKERNEL));
    cudaErrCheck(cudaEventSynchronize(stopKERNEL));
    cudaErrCheck(cudaEventElapsedTime(&kernel_time, startKERNEL, stopKERNEL));
