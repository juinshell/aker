...
__global__ void ori_spmv_int(int *dst_vector,
						const int *d_data,const int *d_index, const int *d_perm,
						const int *x_vec,const int *d_nzcnt, const int dim, int iteration) {
    for (int loop = 0; loop < iteration; loop++) {
        int ix=blockIdx.x*blockDim.x+threadIdx.x;
        int warp_id=ix>>5;
        if(ix<dim)
...
__global__ void ptb_spmv_int(int *dst_vector,
						const int *d_data,const int *d_index, const int *d_perm,
						const int *x_vec,const int *d_nzcnt, const int dim,
                        int grid_dimension_x, int grid_dimension_y, int block_dimension_x, int block_dimension_y, int iteration) {
    unsigned int block_pos = blockIdx.x;
    int thread_id_x = threadIdx.x % block_dimension_x;
    // int thread_id_y = threadIdx.x / block_dimension_x;
