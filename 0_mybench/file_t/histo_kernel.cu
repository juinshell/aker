...
__device__ void ptbClearMemory (unsigned int smem[KB][256], int thread_id_x, int block_dimension_x) {
    for (int i = thread_id_x; i < BINS_PER_BLOCK / 4; i += block_dimension_x) {
        ((unsigned int*)smem)[i] = 0;
    }
}


__device__ void ptbCopyMemory (unsigned int *dst, unsigned int src[KB][256], int thread_id_x, int block_dimension_x) {
    for (int i = thread_id_x; i < BINS_PER_BLOCK / 4; i += block_dimension_x)
    {
        dst[i] = ((unsigned int*)src)[i];
    }
...
__global__ void ptb_histo (
    ...
    for (;; block_pos += gridDim.x) {
        ...
        for (int loop = 0; loop < iteration; loop++) {
            ...
            /* Each threadblock contributes to a specific 24KB range of histogram,
                * and also scans every N-th line for interesting data.  N = grid_dimension_x
                */
            unsigned int local_scan_range = sm_range_min + block_id_y;
            unsigned int local_scan_load = block_id_x * block_dimension_x + thread_id_x;

            ptbClearMemory (sub_histo, thread_id_x, block_dimension_x);
            __syncthreads();
            ...
            if (block_id_y == 0)
            {
                ...
                while (local_scan_load < num_elements)
                {
                    /* Read buffer */
                    uchar4 sm = sm_mappings[local_scan_load];
                    local_scan_load += block_dimension_x * grid_dimension_x;
            ...
            /* Store sub histogram to global memory */
            unsigned int store_index = block_id_x * (histo_height * histo_width / 4) + (local_scan_range * BINS_PER_BLOCK / 4);

            __syncthreads();
            ptbCopyMemory (&(global_subhisto[store_index]), sub_histo, thread_id_x, block_dimension_x);
        }
    }
}


__device__ void mix_histo (
    uchar4 *sm_mappings,
    unsigned int num_elements,
    unsigned int sm_range_min,
    unsigned int sm_range_max,
