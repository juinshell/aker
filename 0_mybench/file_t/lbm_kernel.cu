...
__global__ void ptb_lbm_int( int* srcGrid, int* dstGrid,
    int grid_dimension_x, int grid_dimension_y, int grid_dimension_z,
    int block_dimension_x, int block_dimension_y, int block_dimension_z, int iteration) {
	//Using some predefined macros here.  Consider this the declaration 
    //  and initialization of the variables SWEEP_X, SWEEP_Y and SWEEP_Z
    ...
    SWEEP_VAR
	int temp_swp, tempC, tempN, tempS, tempE, tempW, tempT, tempB;
	int tempNE, tempNW, tempSE, tempSW, tempNT, tempNB, tempST ;
	int tempSB, tempET, tempEB, tempWT, tempWB;
    ...
    for (;; block_pos += gridDim.x) {
        ...
        for (int loop = 0; loop < iteration; loop++) {
            ...
            if( TEST_FLAG_SWEEP( srcGrid, OBSTACLE )) {
                ...
                tempC = temp2*tempC + temp1*(                                 - u2);
                    temp1 = DFL2*temp_base;	
                tempN = temp2*tempN + temp1*(       uy*(4*uy       + 3) - u2);
                tempS = temp2*tempS + temp1*(       uy*(4*uy       - 3) - u2);
                tempT = temp2*tempT + temp1*(       uz*(4*uz       + 3) - u2);
                tempB = temp2*tempB + temp1*(       uz*(4*uz       - 3) - u2);
                tempE = temp2*tempE + temp1*(       ux*(4*ux       + 3) - u2);
                tempW = temp2*tempW + temp1*(       ux*(4*ux       - 3) - u2);
                temp1 = DFL3*temp_base;
                tempNT= temp2*tempNT + temp1 *( (+uy+uz)*(4*(+uy+uz) + 3) - u2);
                tempNB= temp2*tempNB + temp1 *( (+uy-uz)*(4*(+uy-uz) + 3) - u2);
                tempST= temp2*tempST + temp1 *( (-uy+uz)*(4*(-uy+uz) + 3) - u2);
                tempSB= temp2*tempSB + temp1 *( (-uy-uz)*(4*(-uy-uz) + 3) - u2);
                tempNE = temp2*tempNE + temp1 *( (+ux+uy)*(4*(+ux+uy) + 3) - u2);
                tempSE = temp2*tempSE + temp1 *((+ux-uy)*(4*(+ux-uy) + 3) - u2);
                tempET = temp2*tempET + temp1 *( (+ux+uz)*(4*(+ux+uz) + 3) - u2);
                tempEB = temp2*tempEB + temp1 *( (+ux-uz)*(4*(+ux-uz) + 3) - u2);
                tempNW = temp2*tempNW + temp1 *( (-ux+uy)*(4*(-ux+uy) + 3) - u2);
                tempSW = temp2*tempSW + temp1 *( (-ux-uy)*(4*(-ux-uy) + 3) - u2);
                tempWT = temp2*tempWT + temp1 *( (-ux+uz)*(4*(-ux+uz) + 3) - u2);
                tempWB = temp2*tempWB + temp1 *( (-ux-uz)*(4*(-ux-uz) + 3) - u2);
            }
