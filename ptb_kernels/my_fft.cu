...
int main( int argc, char **argv)
{
	...
	// GPTB Running
	fft_block.x = ori_fft_block.x * ori_fft_block.y * ori_fft_block.z;
	fft_block.y = 1;
	fft_block.z = 1;
	printf("[GPTB] Running with fft...\n");
	printf("[GPTB] fft_grid -- %d * %d * %d fft_block -- %d * %d * %d\n", 
		fft_grid.x, fft_grid.y, fft_grid.z, fft_block.x, fft_block.y, fft_block.z);
	
	cudaErrCheck(cudaEventRecord(startKERNEL));
	checkKernelErrors((general_ptb_fft<<<fft_grid, fft_block>>>(fft_gptb_source, 
		ori_fft_grid.x, ori_fft_grid.y, ori_fft_grid.z, ori_fft_block.x, ori_fft_block.y, ori_fft_block.z,
		0, fft_grid.x * fft_grid.y * fft_grid.z, ori_fft_grid.x * ori_fft_grid.y * ori_fft_grid.z, 0)));
	cudaErrCheck(cudaEventRecord(stopKERNEL));
	cudaErrCheck(cudaEventSynchronize(stopKERNEL));
	cudaErrCheck(cudaEventElapsedTime(&kernel_time, startKERNEL, stopKERNEL));
