...
int main(int argc, char* argv[]) {
    ...
    // GPTB
    // ---------------------------------------------------------------------------------
        tpacf_block.x = ori_tpacf_block.x * ori_tpacf_block.y * ori_tpacf_block.z;
        tpacf_block.y = tpacf_block.z = 1;
        printf("[GPTB] Running with tpacf...\n");
        printf("[GPTB] tpacf_grid -- %d * %d * %d tpacf_block -- %d * %d * %d \n", 
            tpacf_grid.x, tpacf_grid.y, tpacf_grid.z, tpacf_block.x, tpacf_block.y, tpacf_block.z);
        
        cudaMemcpyToSymbol(dev_binb, binb, (NUM_BINS+1)*sizeof(float));
        cudaErrCheck(cudaEventRecord(startKERNEL));
        checkKernelErrors((general_ptb_tpacf <<< tpacf_grid, tpacf_block >>> (tpacf_gptb_hists, tpacf_gptb_x, tpacf_gptb_y, tpacf_gptb_z, NUM_SETS, NUM_ELEMENTS, 
            ori_tpacf_grid.x, ori_tpacf_grid.y, ori_tpacf_grid.z, ori_tpacf_block.x, ori_tpacf_block.y, ori_tpacf_block.z,
            0, tpacf_grid.x * tpacf_grid.y * tpacf_grid.z, ori_tpacf_grid.x * ori_tpacf_grid.y * ori_tpacf_grid.z, 0)));
        cudaErrCheck(cudaEventRecord(stopKERNEL));
        cudaErrCheck(cudaEventSynchronize(stopKERNEL));
        cudaErrCheck(cudaEventElapsedTime(&kernel_time, startKERNEL, stopKERNEL));
        printf("[GPTB] tpacf took %f ms\n\n", kernel_time);
    // ---------------------------------------------------------------------------------------
