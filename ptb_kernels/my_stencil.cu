...
int main(int argc, char* argv[]) {
    ...
    // GPTB
    // ---------------------------------------------------------------------------------------
        dim3 stencil_gptb_grid(SM_NUM * 3, 1, 1);
        dim3 stencil_gptb_block(128, 1, 1);
        // stencil_block.x = stencil_block_dim_x * stencil_block_dim_y;
        // stencil_block.y = 1;
        
        printf("[GPTB] Running with stencil...\n");
        printf("[GPTB] stencil_grid -- %d * %d * %d stencil_block -- %d * %d * %d \n", 
            stencil_gptb_grid.x, stencil_gptb_grid.y, stencil_gptb_grid.z, stencil_gptb_block.x, stencil_gptb_block.y, stencil_gptb_block.z);

        cudaErrCheck(cudaEventRecord(startKERNEL));
        checkKernelErrors(
                (general_ptb_stencil<<<stencil_gptb_grid, stencil_gptb_block>>>(c0, c1, stencil_gptb_a0, stencil_gptb_anext, nx, ny, nz,
                    ori_stencil_grid.x, ori_stencil_grid.y, ori_stencil_grid.z, ori_stencil_block.x, ori_stencil_block.y, ori_stencil_block.z,
                    0, stencil_gptb_grid.x * stencil_gptb_grid.y * stencil_gptb_grid.z, ori_stencil_grid.x * ori_stencil_grid.y * ori_stencil_grid.z, 0)));
        cudaErrCheck(cudaEventRecord(stopKERNEL));
        cudaErrCheck(cudaEventSynchronize(stopKERNEL));
        cudaErrCheck(cudaEventElapsedTime(&kernel_time, startKERNEL, stopKERNEL));
    ...
    // ---------------------------------------------------------------------------------------
        ...
        cudaErrCheck(cudaMemcpy(host_stencil_gptb_a0, stencil_gptb_a0, nx * ny * nz * sizeof(float), cudaMemcpyDeviceToHost));
        int errors = 0;
        ...
        for (int i = 0; i < nx * ny * nz; i++) {
            float v1 = host_stencil_ori_a0[i];
            float v2 = host_stencil_gptb_a0[i];
            if (fabs(v1 - v2) > 0.001f) {
            errors++;
            if (errors < 10) printf("%f %f\n", v1, v2);
            }
    ...
    cudaErrCheck(cudaFree(stencil_gptb_a0));
    cudaErrCheck(cudaFree(stencil_gptb_anext));

    free(host_stencil_ori_a0);
    free(host_stencil_ptb_a0);
    free(host_stencil_gptb_a0);
