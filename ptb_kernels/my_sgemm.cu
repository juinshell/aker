...
int main(int argc, char* argv[]) {
    ...
    // GPTB running
        sgemm_block.x = ori_sgemm_block.x * ori_sgemm_block.y * sgemm_block.z;
        sgemm_block.y = sgemm_block.z = 1;
        printf("[GPTB] Running with sgemm...\n");
        printf("[GPTB] sgemm_grid -- %d * %d sgemm_block -- %d * %d \n", 
            sgemm_grid.x, sgemm_grid.y, sgemm_block.x, sgemm_block.y);
        
        cudaErrCheck(cudaEventRecord(startKERNEL));
        checkKernelErrors((general_ptb_sgemm <<< sgemm_grid, sgemm_block >>> (
            ...
            0, sgemm_grid.x * sgemm_grid.y * sgemm_grid.z, ori_sgemm_grid.x * ori_sgemm_grid.y * ori_sgemm_grid.z, 0)));
        cudaErrCheck(cudaEventRecord(stopKERNEL));
        cudaErrCheck(cudaEventSynchronize(stopKERNEL));
        cudaErrCheck(cudaEventElapsedTime(&kernel_time, startKERNEL, stopKERNEL));
