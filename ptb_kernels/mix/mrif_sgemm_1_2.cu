...
int main(int argc, char* argv[]) {
  ...
  // ---------------------------------------------------------------------------------------
        ...
        cudaErrCheck(cudaEventRecord(startKERNEL));
        checkKernelErrors((mixed_mrif_sgemm_kernel_1_2 <<<mix_kernel_grid, mix_kernel_block>>>(mrif_numK, FHGridBase, gptb_x, gptb_y, gptb_z, gptb_outR, gptb_outI, 
        ...
        cudaErrCheck(cudaEventSynchronize(stopKERNEL));
        cudaErrCheck(cudaEventElapsedTime(&kernel_time, startKERNEL, stopKERNEL));
        printf("[MIX] mrif_sgemm 1_2 took %f ms\n\n", kernel_time);
  // ---------------------------------------------------------------------------------------
