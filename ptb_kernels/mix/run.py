...
k_list = ["cp", "cutcp", "fft", "lbm", "mrif", "mriq", "sgemm"]
    ...
    for executable_name in exe_list:
        ...
        try:
            ...
            if data_ori1.__len__() != num_repetitions or data_ori2.__len__() != num_repetitions or data_ptb1.__len__() != num_repetitions or data_ptb2.__len__() != num_repetitions or data_mix.__len__() != num_repetitions:
                print(f"Error running {executable_name}, data length not equal to num_repetitions, Exit!")
            ...
            if abs(data_ori1[start_index] - data_ori1[end_index - 1]) > 0.1:
                print(f"Warning: {executable_name} ORI1 has large variance")
            if abs(data_ori2[start_index] - data_ori2[end_index - 1]) > 0.1:
                print(f"Warning: {executable_name} ORI2 has large variance")
            if abs(data_ptb1[start_index] - data_ptb1[end_index - 1]) > 0.1:
                print(f"Warning: {executable_name} PTB1 has large variance")
            if abs(data_ptb2[start_index] - data_ptb2[end_index - 1]) > 0.1:
                print(f"Warning: {executable_name} PTB2 has large variance")
            if abs(data_mix[start_index] - data_mix[end_index - 1]) > 0.1:
                print(f"Warning: {executable_name} MIX has large variance")
