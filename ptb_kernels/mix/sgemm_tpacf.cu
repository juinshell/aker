...
#include "mix_kernel/sgemm-tpacf.cu" 
...
int main(int argc, char* argv[]) {
    ...
    // free var in ptb and ori
    // ---------------------------------------------------------------------------------------
        cudaErrCheck(cudaFree(tpacf_ptb_x));
        cudaErrCheck(cudaFree(tpacf_ptb_y));
        cudaErrCheck(cudaFree(tpacf_ptb_z));
        cudaErrCheck(cudaFree(tpacf_ptb_hists));

        cudaErrCheck(cudaFree(tpacf_ori_x));
        cudaErrCheck(cudaFree(tpacf_ori_y));
        cudaErrCheck(cudaFree(tpacf_ori_z));
        cudaErrCheck(cudaFree(tpacf_ori_hists));

    // ---------------------------------------------------------------------------------------
  // MIX
  // ---------------------------------------------------------------------------------------
        dim3 mix_kernel_grid = dim3(SM_NUM / 2, 1, 1);
        dim3 mix_kernel_block = dim3(384, 1, 1);
        cudaErrCheck(cudaEventRecord(startKERNEL));
        checkKernelErrors((mixed_sgemm_tpacf_kernel <<<mix_kernel_grid, mix_kernel_block>>>(sgemm_gptb_a, sgemm_gptb_b, sgemm_gptb_c, NORMAL_M, NORMAL_N, NORMAL_K,
            ori_sgemm_grid.x, ori_sgemm_grid.y, ori_sgemm_grid.z, ori_sgemm_block.x, ori_sgemm_block.y, ori_sgemm_block.z,
            0, mix_kernel_grid.x * mix_kernel_grid.y * mix_kernel_grid.z, ori_sgemm_grid.x * ori_sgemm_grid.y * ori_sgemm_grid.z, tpacf_gptb_hists, tpacf_gptb_x, tpacf_gptb_y, tpacf_gptb_z, NUM_SETS, NUM_ELEMENTS, 
        ...
        cudaErrCheck(cudaEventSynchronize(stopKERNEL));
        cudaErrCheck(cudaEventElapsedTime(&kernel_time, startKERNEL, stopKERNEL));
        printf("[MIX] sgemm_tpacf took %f ms\n\n", kernel_time);
  // ---------------------------------------------------------------------------------------
