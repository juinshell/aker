...
int main(int argc, char* argv[]) {
    ...
    // GPTB running
        mriq_block2.x = ori_mriq_block2.x * ori_mriq_block2.y * ori_mriq_block2.z;
        mriq_block2.y = mriq_block2.z = 1;
        printf("[GPTB] Running with mriq...\n");
        printf("[GPTB] mriq_grid2 -- %d * %d * %d mriq_block2 -- %d * %d * %d \n", 
            mriq_grid2.x, mriq_grid2.y, mriq_grid2.z, mriq_block2.x, mriq_block2.y, mriq_block2.z);

        cudaErrCheck(cudaEventRecord(startKERNEL));
        checkKernelErrors((general_ptb_mriq <<< mriq_grid2, mriq_block2 >>>(numK, QGridBase, 
            ...
            0, mriq_grid2.x * mriq_grid2.y * mriq_grid2.z, ori_mriq_grid2.x * ori_mriq_grid2.y * ori_mriq_grid2.z, 0)));


    // Checking results
    // ---------------------------------------------------------------------------------------
        cudaMemcpy(host_mriq_ori_Qi, mriq_ori_Qi, numK * sizeof(float), cudaMemcpyDeviceToHost);
        cudaMemcpy(host_mriq_ptb_Qi, mriq_ptb_Qi, numK * sizeof(float), cudaMemcpyDeviceToHost);
        cudaMemcpy(host_mriq_gptb_Qi, mriq_gptb_Qi, numK * sizeof(float), cudaMemcpyDeviceToHost);

        int errors = 0;
        for (int i = 0; i < numX; i++) {
            float v1 = host_mriq_ori_Qi[i];
            float v2 = host_mriq_ptb_Qi[i];
