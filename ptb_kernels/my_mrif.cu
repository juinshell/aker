...
int main (int argc, char *argv[]) {
    ...
    // ---------------------------------------------------------------------------------------
        ...
        cudaErrCheck(cudaMalloc((void **)&gptb_x, numX * sizeof(float)));
        cudaErrCheck(cudaMalloc((void **)&gptb_y, numX * sizeof(float)));
        cudaErrCheck(cudaMalloc((void **)&gptb_z, numX * sizeof(float)));
        cudaErrCheck(cudaMemcpy(gptb_x, base_x, numX * sizeof(float), cudaMemcpyHostToDevice));
        cudaErrCheck(cudaMemcpy(gptb_y, base_y, numX * sizeof(float), cudaMemcpyHostToDevice));
        cudaErrCheck(cudaMemcpy(gptb_z, base_z, numX * sizeof(float), cudaMemcpyHostToDevice));
        cudaErrCheck(cudaMalloc((void **)&gptb_outR, numX * sizeof(float)));
        cudaErrCheck(cudaMalloc((void **)&gptb_outI, numX * sizeof(float)));
        cudaErrCheck(cudaMemset(gptb_outR, 0, numX * sizeof(float)));
        cudaErrCheck(cudaMemset(gptb_outI, 0, numX * sizeof(float)));

        host_ori_outI = (float*) calloc (numX, sizeof (float));
        host_ptb_outI = (float*) calloc (numX, sizeof (float));
        host_gptb_outI = (float*) calloc (numX, sizeof (float));
    // ---------------------------------------------------------------------------------------
    ...
    // GPTB running
    // ---------------------------------------------------------------------------------------
        mrif_block2.x = ori_mrif_block2.x * ori_mrif_block2.y * ori_mrif_block2.z;
        mrif_block2.y = mrif_block2.z = 1;
        printf("[GPTB] Running with mrif...\n");
        printf("[GPTB] mrif_grid2 -- %d * %d * %d mrif_block2 -- %d * %d * %d \n", 
            mrif_grid2.x, mrif_grid2.y, mrif_grid2.z, mrif_block2.x, mrif_block2.y, mrif_block2.z);

        cudaErrCheck(cudaEventRecord(startKERNEL));
        checkKernelErrors((general_ptb_mrif <<< mrif_grid2, mrif_block2 >>> (numK, FHGridBase, gptb_x, gptb_y, gptb_z, gptb_outR, gptb_outI, 
            ori_mrif_grid2.x, ori_mrif_grid2.y, ori_mrif_grid2.z, ori_mrif_block2.x, ori_mrif_block2.y, ori_mrif_block2.z,
            0, mrif_grid2.x * mrif_grid2.y * mrif_grid2.z, ori_mrif_grid2.x * ori_mrif_grid2.y * ori_mrif_grid2.z, 0)));
        cudaErrCheck(cudaEventRecord(stopKERNEL));
        cudaErrCheck(cudaEventSynchronize(stopKERNEL));
        cudaErrCheck(cudaEventElapsedTime(&kernel_time, startKERNEL, stopKERNEL));
    ...
    // ---------------------------------------------------------------------------------------
        ...
        cudaErrCheck(cudaMemcpy(host_gptb_outI, gptb_outI, numX * sizeof(float), cudaMemcpyDeviceToHost));
        int errors = 0;
