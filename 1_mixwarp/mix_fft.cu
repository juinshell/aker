...
int main(int argc, char* argv[]) {
	...
	if (mixwarp == 1) {
		...
		checkKernelErrors((ptb_tzgemm<<<wmma_grid, wmma_block, SHMEM_SZ, streams[0]>>>(
				...
				// alpha, beta,
				wmma_grid_dim_x, wmma_block_dim_x, wmma_iter)));
		checkKernelErrors((ptb_fft<<<fft_grid, fft_block, 0, streams[1]>>>(fft_ori_source, 
            	fft_grid_dim_x, fft_block_dim_x, fft_iter))); 	
		...
		checkKernelErrors((ptb_tzgemm<<<wmma_grid, wmma_block, SHMEM_SZ, streams[0]>>>(
				...
				// alpha, beta,
				wmma_grid_dim_x, wmma_block_dim_x, wmma_iter)));
		checkKernelErrors((ori_fft<<<fft_grid, fft_block, 0, streams[1]>>>(fft_ori_source, fft_iter))); 	
