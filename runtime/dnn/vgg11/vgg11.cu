...
void VGG11::initParams() {
    ...
    CUDA_SAFE_CALL(cudaMallocHost((void**)&Result_99_0_host, sizeof(float) * 1001));

    // fill input values
    for (int i = 0; i < 150528; ++i) Parameter_32_0_host[i] = 1.0f;

    CUDA_SAFE_CALL(cudaMemcpy(Parameter_32_0, Parameter_32_0_host, sizeof(float) * 150528, cudaMemcpyHostToDevice));
